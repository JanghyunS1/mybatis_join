...
public class UserController {
	...
	public String loginView(@ModelAttribute("user") UserVO vo) {
		System.out.println("로그인 처리 - Spring MVC 호출 - Controller 통합");
		...
		/*
			@ModelAttribute : command 객체로 던져지는 객체 이름을 다른이름으로 변경 (UserVO => 
	...
	// 2. 로그아웃 (POST 방식 처리 메소드)
	@RequestMapping(value="/logout.do")
	public String logout(HttpSession session) {
		System.out.println("로그 아웃 처리 - Spring MVC Annotation호출 - Controller 통합");
	...
	public String insertView(UserVO vo) {
		...
		// 회원 등록 폼에서 값을 넣고 전송 : DB에 저장 <== POST
